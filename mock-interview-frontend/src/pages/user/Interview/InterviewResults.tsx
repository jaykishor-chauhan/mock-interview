          ...
          <TabsContent value="questions" className="space-y-6">
            ...
            {receivedResponses.length > 0 && receivedResponses.map((resp: any, idx: number) => {
              const timeStr = resp?.timestamp
                ...
                : "";
              return (
                <Card key={`resp-${idx}`} className="shadow-lg">
                  <CardHeader>
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                      <div className="flex-1">
                        <CardTitle className="text-base sm:text-lg leading-relaxed mb-3">{resp.question}</CardTitle>
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <span className="font-medium">Your answer:</span>
                          <span>{resp.answer}</span>
                        </div>
                        {timeStr && <div className="text-xs text-muted-foreground mt-2">Answered: {timeStr}</div>}
                      </div>
                    </div>
                  </CardHeader>
                </Card>
